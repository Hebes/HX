        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                if (GUILayout.Button("获取前缀", EditorStyles.miniButtonMid))
                    ACHierarchyToolReName_Prefix1 = ToolExpansion_Find.ACGetObj().ACGetPrefix();
                if (GUILayout.Button("前缀添加", EditorStyles.miniButtonMid))
                    ToolExpansion_Find.ACGetObjs().ACChangePrefixLoop(ACHierarchyToolReName_Prefix1);
                if (GUILayout.Button("去除前缀", EditorStyles.miniButtonMid))
                    ToolExpansion_Find.ACGetObjs().ACChangePrefixLoop(ACHierarchyToolReName_Prefix1, false);
            }
            EditorGUILayout.EndHorizontal();
        }
    }
