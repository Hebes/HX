...
{
    public bool SINGMASK_SYSTEM;//是否启用单遮模式 "窗口遮罩模式", "True：开启单遮罩模式(多个窗口叠加只有一个Mask遮罩，透明度唯一) False:开启叠着模式(一个窗口一个单独的Mask遮罩，透明度叠加)"
    ...
    public GameObject LoadWindow(string wndNamePath)
    {
        GameObject window = GameObject.Instantiate<GameObject>(Resources.Load<GameObject>(wndNamePath), mUIRoot);
