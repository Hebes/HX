        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.ProgressBar(position, animationUI.CurrentTime / animationUI.TotalDuration,
                Mathf.Clamp((Mathf.Round(animationUI.CurrentTime * 100) / 100), 0, 100)
                    .ToString() + "/" + animationUI.TotalDuration.ToString() + " Seconds, [" +
                (Mathf.Round(animationUI.CurrentTime / animationUI.TotalDuration * 10000) / 100)
                .ToString() + "%]"
            );
            ...
            if (animationUI.AnimationSequence.Length > 0)
                if (animationUI.AnimationSequence[0].PropertyRectY < 115)
            ...
            foreach (Sequence sequence in animationUI.AnimationSequence)
            {
                ...
                {
                    float currentSideLoadingRectHeight = (sequence.Duration == 0)
                        ? currentRect.height
                        : Mathf.Lerp(0, currentRect.height, (animationUI.CurrentTime - sequence.StartTime) / sequence.Duration);
