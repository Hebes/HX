...
namespace Framework.Core
{
    public static partial class Utility
    {
        ...
        public static class Reflection
        {
            ...
            /// 获取注解
            /// </summary>
            public static List<T> GetAttribute<T>(string assemblyName = "Assembly-CSharp") where T : Attribute
            {
                List<T> attributeList = new();
                Assembly assembly = Assembly.Load(assemblyName);
                //var assembly = Assembly.GetExecutingAssembly(); // 获取当前程序集
                var typeArray = assembly.GetTypes(); // 获取程序集中的所有类型
                foreach (var type in typeArray)
                {
                    //确定是否将指定类型的任意自定义属性应用于程序集、模块、类型成员或方法参数。
                    if (!Attribute.IsDefined(type, typeof(T))) continue;
                    var attribute = Attribute.GetCustomAttribute(type, typeof(T)); //获取Attribute
            ...
            {
                return objs == null ? Activator.CreateInstance(type) : Activator.CreateInstance(type, objs);
            }
            ...
            public static IEnumerable<Type> GetSonClass(Type typeBase, string assemblyName = "Assembly-CSharp")
            {
                ...
                foreach (var type in allType)
                {
                    ...
                    if (!type.IsSubclassOf(typeBase)) continue;
                    types.Add(type);
