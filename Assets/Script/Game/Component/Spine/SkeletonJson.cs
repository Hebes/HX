...
namespace Spine
{
	public class SkeletonJson
	{
		...
		public SkeletonData ReadSkeletonData(TextReader reader)
		{
			...
			if (dictionary.ContainsKey("slots"))
			{
				foreach (object obj4 in ((List<object>)dictionary["slots"]))
				{
					...
					{
						slotData.blendMode = (BlendMode)Enum.Parse(typeof(BlendMode), (string)dictionary5["blend"], false);
					}
					else
					{
		...
		private Attachment ReadAttachment(Skin skin, string name, Dictionary<string, object> map)
		{
			...
			{
				attachmentType = (AttachmentType)Enum.Parse(typeof(AttachmentType), (string)map["type"], false);
			}
			string path = name;
		...
		private void ReadAnimation(string name, Dictionary<string, object> map, SkeletonData skeletonData)
		{
				...
				foreach (object obj8 in list7)
				{
					...
					if (dictionary10.ContainsKey("offsets"))
					{
						...
						foreach (object obj9 in list8)
						{
							...
							int num17 = m + (int)((float)dictionary11["offset"]);
							array2[num17] = m++;
						}
						while (m < count3)
						{
			...
			if (map.ContainsKey("events"))
			{
				...
				foreach (object obj10 in list9)
				{
					...
					eventTimeline.SetFrame(num18++, (float)dictionary12["time"], @event);
				}
