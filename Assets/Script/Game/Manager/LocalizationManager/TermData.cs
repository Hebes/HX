...
public class TermData
{
    public string GetTranslation(int idx)
    {
        ...
        {
            return string.IsNullOrEmpty(this.Languages_Touch[idx]) ? this.Languages[idx] : this.Languages_Touch[idx];
        }
    ...
    public bool HasTouchTranslations()
    {
        ...
        while (i < num)
        {
            if (!string.IsNullOrEmpty(this.Languages_Touch[i]) && !string.IsNullOrEmpty(this.Languages[i]) &&
                this.Languages_Touch[i] != this.Languages[i])
            {
                return true;
            }
